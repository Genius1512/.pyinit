...
pyinstaller --noconfirm --onefile --icon "$(cd $(dirname $(dirname "${BASH_SOURCE[0]}")) && pwd)/icon.ico" --noconsole --name $name "$(cd $(dirname $(dirname "${BASH_SOURCE[0]}")) && pwd)/scripts/__main__.py"
